...
int get_new_UID(void)
{
	...
	if(pthread_mutex_lock(&cookie_mutex) != 0)
		fail_errno("incapache-students: pthread_mutex_lock error in get_new_UID");
	...
	if(pthread_mutex_unlock(&cookie_mutex) != 0)
		fail_errno("incapache-students: pthread_mutex_unlock error in get_new_UID");
...
int keep_track_of_UID(int myUID)
{
	...
	// In dubbio
	if(pthread_mutex_lock(&cookie_mutex) != 0)
		fail_errno("incapache-students: pthread_mutex_lock error in keep_track_of_UID");

	UserTracker[myUID]++;
	newcount = UserTracker[myUID];

	if(pthread_mutex_unlock(&cookie_mutex) != 0)
		fail_errno("incapache-students: pthread_mutex_unlock error in keep_track_of_UID");
									 ...
									 char *filename, struct stat *stat_p)
{
	...
	if(gmtime_r(&now_t, &now_tm)==NULL)
		fail_errno("incapache-students: gmtime error in send_response");
	...
	case RESPONSE_CODE_NOT_FOUND:
		...
		if ((fd = open(HTML_404, O_RDONLY)) >= 0)
		{
			...
			if(stat_p == NULL){
				// ... assegnamo il buffer per contenere i dati a stat_p ...
				stat_p = &stat_buffer;
					
				// ... e recuperiamo i metadati di HTML_404
				if(stat(HTML_404, stat_p) == -1)
					fail_errno("incapache-students: error in stat HTML_404");
			}
		...
		if ((fd = open(HTML_501, O_RDONLY)) >= 0)
		{
			...
			if(stat_p == NULL){
				...
				if(stat(HTML_501, stat_p) == -1)
					fail_errno("incapache-students: error in stat HTML_501");
			}
	...
	if (cookie >= 0)
	{
		...
		/*** TO BE DONE 6.0 START ***/
		//se lo commentiamo reagisce in modo anomalo
		cookie = get_new_UID();
	...
	if (file_size > 0 && mime_type != NULL)
	{
		...
		if(gmtime_r(&file_modification_time, &file_modification_tm)==NULL)
			fail_errno("incapache-students: gmtime error: ");
	...
	if (fd >= 0)
	{
		...
		if(header_sent = sendfile(client_fd, fd, NULL, file_size) == -1)
			fail_errno("incapache-students: sendfile error in http");

		if(close(fd) == -1)
			fail_errno("incapache-students: close fd error in http");
