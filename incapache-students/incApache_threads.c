...
/*** TO BE DONE 6.1 START ***/
	...
	// Questa join si usa sul thread di connessione, che dovrà aspettare che terminino prima tutti gli altri thread di
	// risposta prima di proseguire a partire dal più recente inserito
	//
	// Viene usata quando una connessione deve terminare (quindi quando il file descriptor per la connessione si andrà a
	// chiudere)
	//
	// Se nella to_join del thread di connessione c'è qualcosa, significa che allora bisogna controllare l'effettiva
	// terminazione dei thread di risposta

	if(to_join[conn_no] != NULL)
	{
		// Cerchiamo la posizione del thread puntato dalla to_join del thread di connessione
		for(i = MAX_CONNECTIONS; i < MAX_THREADS && to_join[conn_no] != &thread_ids[i]; ++i);
		
		// Si fa la join per aspettare la terminazione dei thread di risposta (diventa una specie di catena dove si
		// aspetta che ci sia una terminazione a cascata fra i thread di risposta, ed infine in quello di connessione)
		if(pthread_join(thread_ids[i], NULL) == -1)
			fail_errno("incapache-students: error in pthread_join");
		
		// Essendo che lavoriamo su strutture dati condivise fra vari thread, usiamo i mutex per evitare potenziali
		// race conditions
		if(pthread_mutex_lock(&threads_mutex) != 0)
			fail_errno("incapache-students: error while locking threads_mutex");
		
		// Aggiorno il numero di response_threads su connnection_no...
		--no_response_threads[conn_no];
		// Ed aggiorno anche il numero di thread liberi su connection_no
		++no_free_threads;
		
		// Reimpostiamo il connection_no del primo response thread a FREE_SLOT
		connection_no[i] = FREE_SLOT;
		
		// Ripristiniamo la to_join[conn_no] a NULL (come all'inizializzazione iniziale)
		to_join[conn_no] = NULL;
		
		// Sblocchiamo il mutex
		if(pthread_mutex_unlock(&threads_mutex) != 0)
			fail_errno("incapache-students: error while unlocking threads_mutex");
	}
...
/*** TO BE DONE 6.1 START ***/
	...
	// Controlliamo se il thread attualmente ha qualche thread precedente nella to_join[thrd_no]...
	
	if(to_join[thrd_no] != NULL)
	{
		// Se ha un thread precedente, allora bisogna cercarlo a tappeto sulla struttura usando un ciclo for che
		// continua fnchè non lo trova (o finchè non arriva al numero = a MAX_THREADS)...
		
		for(i = 0; i < MAX_THREADS && to_join[thrd_no] != &thread_ids[i]; ++i);
		
		// Quando lo troviamo aspettiamo che termini usando la syscall pthread_join()...
		...
		// Aggiorno il numero dei response thread usati dalla connessione
		--no_response_threads[connection_no[thrd_no]];
		// Aggiorno il numero di thread liberi nella connessione
		++no_free_threads;
		
		// Ripristiniamo come FREE_SLOT il numero di connessione del thread che è stato liberato
		connection_no[i] = FREE_SLOT;
...
/*** TO BE DONE 6.1 START ***/
	...
	// Dovendo inizializzare il thread di connessione, siamo costretti a dover far puntare la sua informazione
	// ausiliaria "to_join[connection_no] come uguale a NULL (perchè appena creiamo un thread di connessione, 
	// non ci sono thread di risposta)"
...
/*** TO BE DONE 6.0 START ***/
	...
	// Blocchiamo il mutex su mime_mutex
	if(pthread_mutex_lock(&mime_mutex) != 0)
		fail_errno("incapache-students: error locking mime_mutex");
...
/*** TO BE DONE 6.0 START ***/
	...
	// Sblocchiamo il mutex su mime_mutex
	if(pthread_mutex_unlock(&mime_mutex) != 0)
		fail_errno("incapache-students: error unlocking mime_mutex");
...
/*** TO BE DONE 6.1 START ***/
	...
	// Si tratta questa "struttura nella struttura" come una lista con punto di ingresso = alla
	// to_join del thread di connessione, dove questo viene inizializzato con un valore a NULL nella
	// client_connection_thread()
	//
	// Vista la sua natura di lista, allora siamo in grado di poter connettere il thread di risposta che vogliamo
	// inserire a ciò che è contenuto attualmente nella to_join del thread di connessione (nel primo caso a NULL,
	// nei casi successivi all'ultimo thread di risposta inserito nella struttura prima del nuovo thread)
	//
	// Inoltre aggiorniamo la to_join[connection_idx] per puntare al nuovo thread inserito nella struttura
